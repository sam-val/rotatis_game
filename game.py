...
normal = Settings(shapes_num=1, moves_num=1, time_reward=20)
easy = Settings(shapes_num=1, moves_num=1, time_reward=25)
hard = Settings(shapes_num=2, moves_num=3, time_reward=15)
...
def finish(lose=True):
    ...
    if lose:
        ...
        label = tkinter.Label(
            root,
            text="You Won!\nTotal time spent:  "
            + display_ob.convert(int(total_elapse))
            + "\nEnter your name here:",
        )
...
def center_tk_window(root, offset_x=0, offset_y=0):
    ...
    x, y = (
        screen_width // 2 - w // 2 + offset_x,
        screen_height // 2 - h // 2 + offset_y,
    )
...
def set_settings():
    ...
    level = current_level
    if level == 0:
        currentSettings["current_shapes_num"] = DIFFICULTY_SETTINGS[
            difficulty
        ].shapes_num
        currentSettings["current_moves_num"] = DIFFICULTY_SETTINGS[difficulty].moves_num
        currentSettings["current_time_reward"] = DIFFICULTY_SETTINGS[
            difficulty
        ].time_reward
    elif level == 4:
        currentSettings["current_moves_num"] = (
            DIFFICULTY_SETTINGS[difficulty].moves_num + 3
        )
        currentSettings["current_shapes_num"] = (
            DIFFICULTY_SETTINGS[difficulty].moves_num + 3
        )
    elif level == 3:
        currentSettings["current_shapes_num"] = (
            DIFFICULTY_SETTINGS[difficulty].shapes_num + 2
        )
        currentSettings["current_moves_num"] = (
            DIFFICULTY_SETTINGS[difficulty].moves_num + 2
        )
    elif level == 2:
        currentSettings["current_moves_num"] = (
            DIFFICULTY_SETTINGS[difficulty].moves_num + 1
        )
    elif level == 1:
        currentSettings["current_shapes_num"] = (
            DIFFICULTY_SETTINGS[difficulty].shapes_num + 1
        )
...
def display_high_scores(*args, **kwargs):
    ...
    h_title = tkinter.Label(
        root, text=f"Top 10 High Scores for {difficulty.capitalize()} Difficulty"
    )
...
def goToChooseDifficulty():
    ...
    while running:
        ...
        display_ob.display_text(
            x=CUBE_WIDTH * (SCREEN_WIDTH / 2),
            y=CUBE_WIDTH * 1.5,
            text=choose_label,
            centeredY=True,
            centeredX=True,
        ...
        display_ob.display_text(
            x=CUBE_WIDTH * (SCREEN_WIDTH / 2),
            y=easy_button.posy + CUBE_WIDTH * 0.5,
            font=font,
            text="EASY",
            colour=BLACK,
            centeredX=True,
        ...
        display_ob.display_text(
            x=CUBE_WIDTH * (SCREEN_WIDTH / 2),
            y=normal_button.posy + CUBE_WIDTH * 0.5,
            font=font,
            text="NORMAL",
            colour=BLACK,
            centeredX=True,
        ...
        display_ob.display_text(
            x=CUBE_WIDTH * (SCREEN_WIDTH / 2),
            y=hard_button.posy + CUBE_WIDTH * 0.5,
            font=font,
            text="HARD",
            colour=BLACK,
            centeredX=True,
...
class Side(Board):
    def __init__(self, screen, x, y, width, height, cube_width, cube_height, rect):
        super().__init__(
            screen, x, y, width, height, cube_width, cube_height, rect, border=False
        )
        ...
        if self.moves != None:
            ...
            for index, move in enumerate(self.moves):
                ...
                if index > width - 1:
                    ...
                    index = index % width
                    new_x = x + index * (self.img_width + self.GAP)
                display_ob.display_sur(
                    x=new_x, y=new_y, sur=img, centeredX=False, centeredY=False
                )
...
field_rect = MyRect(
    ...
    colour=DARK_GREY,
    line_colour=WHITE,
    click_colour=SHAPE_COLOUR,
    func=[field_rect_func],
)
field = Field(
    scr,
    x=CUBE_WIDTH * MENU_W,
    y=CUBE_WIDTH * (SCREEN_HEIGHT - MENU_H),
    width=BOARD_W,
    height=BOARD_H,
    cube_width=CUBE_WIDTH,
    cube_height=CUBE_WIDTH,
...
side_field = Field(
    scr,
    x=0,
    y=CUBE_WIDTH * (SCREEN_HEIGHT - MENU_H + 1),
    width=BOARD_W - 2,
    height=BOARD_H - 2,
    cube_width=CUBE_WIDTH / 7,
    cube_height=CUBE_WIDTH / 7,
    rect=side_field_rect,
    border=False,
)
...
if difficulty is not None:
    currentSettings = {
        "current_shapes_num": DIFFICULTY_SETTINGS[difficulty].shapes_num,
...
while running:
    ...
    side.update()
    count_down_clock()
    click_text = "CLICK ME TO CHECK! (shortcut: space)"
    click_colour = WHITE
    if not paused:
    ...
    pygame.draw.rect(
        ...
        WHITE,
        pygame.Rect(
            0, 0, SCREEN_WIDTH * CUBE_WIDTH, CUBE_WIDTH * (SCREEN_HEIGHT - BOARD_H)
        ),
    )
    ...
    if current_level == 4:
        display_ob.display_text(
            x=CUBE_WIDTH * 1.1,
            y=CUBE_WIDTH * 0.15,
            text="".join([" " for x in level]) + "S",
            centeredY=False,
            centeredX=False,
            colour=YELLOW,
            font=clock_font_bold,
        )
    # display the clock
    display_ob.my_display_clock(
        CUBE_WIDTH * CLOCK_POS_X,
        CUBE_WIDTH * CLOCK_POS_Y,
        secs=remaining_time,
        centeredX=True,
        centeredY=True,
        font=clock_font,
    ...
    if time_reward_fade_ob.on:
        ...
        text = "+" + str(reward)
        time_reward_fade_ob.update(
            font=time_added_font, fading_speed=4, text=text, colour=ORANGE
        )
        display_ob.display_sur(
            x=CUBE_WIDTH * CLOCK_POS_X,
            y=CUBE_WIDTH * CLOCK_POS_Y * 0.3,
            sur=time_reward_fade_ob.text_sur,
            centeredX=True,
            centeredY=True,
        )
    ...
    if result_text_fade_ob.on:
        ...
        result_text_fade_ob.update(
            font=compare_font, colour=colour, fading_speed=4, text=display
        )
        display_ob.display_sur(
            x=CUBE_WIDTH * (SCREEN_WIDTH / 2),
            y=CUBE_WIDTH * 0.5,
            sur=result_text_fade_ob.text_sur,
            centeredY=True,
            centeredX=True,
        )
    if swear_fade_ob.on:
